...
d <- fread('../datta/Bayes_SML_EXP1_140820.csv')
...
dd <- d[, .(mean(HA_INITIAL), mean(HA_END)), .(GROUP, SUBJECT, PHASE, TRIAL, ROT)]
ggplot(data=dd) +
...
## dd <- d[TRIAL %in% 11:190, .(mean(HA_INITIAL), mean(HA_END)), .(GROUP, PHASE, TRIAL, ROT, SIG_MP)]
## ggplot(data=dd) +
